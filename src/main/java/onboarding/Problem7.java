...
public class Problem7 {
    ...
    public static List<Map.Entry<String, Integer>> makeScoreList(Map<String, ArrayList<String>> friendsMap, String user, List<String> visitors) {
        ...
        for(String friend : friendsMap.keySet()) {
            if(friend.equals(user) || friendsMap.getOrDefault(user, new ArrayList<>()).contains(friend)) continue; // user 본인, 이미 친구인 사람 제외
            int score = 0;
        ...
        for(String visitor : visitors) {
            if(friendsMap.getOrDefault(user, new ArrayList<>()).contains(visitor)) continue; // 이미 방문자와 친구라면 추천 친구에서 제외
            int score = 0;
    ...
    public static List<String> solution(String user, List<List<String>> friends, List<String> visitors) {
        ...
        for(int i = 0; i < friendsScoreList.size(); i++) {
            if(i == 5) break; // answer 에는 최대 5명 제한
