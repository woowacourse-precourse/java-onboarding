 ...
 * [o] 시작 면이나 마지막 면이 나오도록 책을 펼치지 않는다.
 * [o] 포비와 크롱이 펼친 페이지가 들어있는 리스트/배열 pobi와 crong이 주어질 때, 포비가 이긴다면 1, 크롱이 이긴다면 2, 무승부는 0, 예외사항은 -1로 return 하도록 solution 메서드를 완성하라.
 */
class Problem1 {
    public static int solution(List<Integer> pobi, List<Integer> crong) {
        try {
            if (!isRandomPage(pobi) || isFirstAndLastPage(pobi) || !isRandomPage(crong) || isFirstAndLastPage(crong)) {
                throw new Exception();
            }
