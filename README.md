...
```
1. 맨 첫번째의 크루 닉네임은 확인 리스트에 저장한다. 
2. 두번째 크루 닉네임부터 각 글자가 확인 리스트에 있는 닉네임에 포함되는지 확인한다.
3. 포함된다면 그 다음 글자도 포함되는지 확인한다.
4. 만약 포함된다면 중복된 닉네임 리스트에 이메일을, 확인 리스트에 닉네임을 저장하고 다음 크루 닉네임으로 넘어간다.
    - 포함이 되지 않는다면 확인리스트에만 저장한다. 
5. 모든 크루들의 닉네임까지 돈다.
6. 중복된 닉네임 리스트를 반환한다.
```
