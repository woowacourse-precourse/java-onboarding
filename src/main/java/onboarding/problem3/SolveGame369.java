...
public class SolveGame369 {
	...
	public Integer count369OfSeparatedNum(Integer gameNum) {
		List<Integer> arrGameNum = makeSequenceGame369Num(gameNum);

		return count369Frequency(arrGameNum);
	}

	public Integer count369Frequency(List<Integer> listGameNum) {
		Integer appear369Count = 0;
		List<Integer> game369Number = Arrays.asList(3, 6, 9);
		for (Integer num : game369Number) {
			appear369Count += Collections.frequency(listGameNum, num);
		}

		return appear369Count;
	}

	private List<Integer> makeSequenceGame369Num(Integer gameNum) {
	...
	public List<Integer> makeSeparatedNum(Integer gameNum) {
