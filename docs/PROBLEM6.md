...
- ⬜ HashMap 객체와 forms 배열을 매개변수로 받아 하여 중복된 닉네임의 이메일들을 ArrayList로 반환하는 duplicatedEmails 메소드 구현
  - ⬜ HashMap의 key값을 닉네임의 부분 문자열로 가지는 forms의 요소값에서 이메일을 HashSet 클래스 객체에 삽입한다
