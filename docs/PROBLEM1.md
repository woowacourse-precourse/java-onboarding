...
- [ ] pobi나 crong이 주운 책이 유효한 지 확인한다.
    - pobi와 crong의 길이가 2인지 확인한다
    - pobi 리스트와 crong 리스트가 연속된 두 페이지인지 확인한다
    - 오른쪽 페이지가 왼쪽 페이지보다 큰 지 확인한다.
- [ ] 유효한 책이라면, 책의 점수를 가져오는 함수를 구현한다.
- [ ] pobi와 crong의 점수를 기반으로 승자를 결정하는 함수를 구현한다. 