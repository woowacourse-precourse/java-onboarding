...
## 문제 6
### 매개변수 처리
- [ ] 크루는 1명 이상, 10,000명 이하인지 확인
- [ ] 이메일 형식에 부합하는지 확인하고, email.com 도메인으로만 제한
- [ ] 전체 길이가 11자 이상, 20자 미만
- [ ] 닉네임은 한글이고, 1자 이상 20자 미만
### 과정
-[ ] forms 를 순회하면서 닉네임을 2글자씩 쪼개서 Map 에 등록
  -[ ] Map 에서 key 는 닉네임 2글자, value는 ArrayList로 이메일을 가지고 있음
  -[ ] 이미 등록된 경우 ArrayList 에 이메일을 추가
-[ ] Map의 value 들을 순회하면서 중복을 제거한 이후 개수가 2개 이상인 경우 이메일들을 result 배열에 추가
-[ ] 이메일 배열을 오름차순으로 정렬하고 중복은 제거하여 리턴
