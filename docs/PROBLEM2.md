...
### 다시 제출한 풀이 과정
처음부터 다시 풀었습니다. 처음에는 연속된 두 문자만 지워지게해서 aabbbcdff같은 경우는 처리하지 못했습니다. 암호를 리스트로 만들어 remove_overlap메서드를 통해 리스트를 돌며 중복문자가 나오면 계속 제거, 나오지 않으면true를 반환하게했습니다. 제거가 끝나면 while문을 빠져나와 다시 문자열로 만들어 반환해주었습니다.
