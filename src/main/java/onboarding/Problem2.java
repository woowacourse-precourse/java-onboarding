...
public class Problem2 {
    ...
    public static String decryptor(String cryptogram) {
        ...
        for (char current : cryptoToArr) {
            if (previous == current) {
                if (resultList.size() == 0) { resultList.add(current); }
                else resultList.remove(resultList.size()-1);

                if(resultList.size() > 0 ) { previous = resultList.get(resultList.size()-1); }
            }
            else {
    ...
    public static String solution(String cryptogram) {
        String answer = decryptor(cryptogram);
        return answer;
    }
}
