...
public class Problem7 {
    ...
    public static HashMap<String, Integer> increaseVisitor(HashMap<String, Integer> ans, String user, HashMap<String, ArrayList<String>> hs, List<String> visitors) {
    ...
    public static List<String> solution(String user, List<List<String>> friends, List<String> visitors) {
        ...
        HashMap<String, Integer> finalAns = increaseVisitor(ans, user, hs, visitors);

        List<String> tmp_answer = new ArrayList<>(ans.keySet());
        List<String> answer = new ArrayList<>();

        Collections.sort(tmp_answer,(s1,s2)-> {     //친구 점수 및 이름순으로 정렬
            return finalAns.get(s2)!=finalAns.get(s1) ? finalAns.get(s2) - finalAns.get(s1) : s1.compareTo(s2);
        });
