...
public class UserManager {
    ...
    public List<String> recommandTo(String targetUser) {
        ...
        return friendsScore.keySet()
                .stream()
                .filter(name -> !isAlreadyFriend(targetUser, name))
                .filter(name -> isScoreOverZeroAt(name))
    ...
    private boolean isAlreadyFriend(String targetUser, String name) {
        return friendsTable.get(targetUser) != null && friendsTable.get(targetUser).contains(name);
    }

    private boolean isScoreOverZeroAt(String name) {
        return friendsScore.get(name) > 0;
    }
