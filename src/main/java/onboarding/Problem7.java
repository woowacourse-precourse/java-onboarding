...
public class Problem7 {
    public static List<String> solution(String targetUser, List<List<String>> friends, List<String> visitors) {
        ...
        LinkedHashMap<String, Integer> userToRecommendScore = new LinkedHashMap<>();
        ...
        friends.stream().filter(f -> {
            ...
            if (!userToFriends.containsKey(friend1)) {
                ...
                // 점수 해시맵 초기화
                if (!targetUserFriends.contains(friend1)) userToRecommendScore.put(friend1, 0);
            }
            ...
            if (!userToFriends.containsKey(friend2)) {
                ...
                if (!targetUserFriends.contains(friend2)) userToRecommendScore.put(friend2, 0);
            }
            userToFriends.get(friend2).add(friend1);
        });

        System.out.println(userToFriends.keySet());
        for (String friend : userToFriends.keySet()) {
            if (targetUserFriends.contains(friend)) {
                for (String user : userToFriends.get(friend)) {
                    Integer score = userToRecommendScore.get(user);
                    userToRecommendScore.put(user, score + 10);
                }
            }
        }
        ...
        for (String visitor : visitors) {
            if (!targetUserFriends.contains(visitor)) { // 친구가 아니어야함
                Integer score = userToRecommendScore.get(visitor);
                if (score == null) {
                    score = 0;
                }
                userToRecommendScore.put(visitor, score + 1);
            }
        }

        return userToRecommendScore.entrySet().stream()
