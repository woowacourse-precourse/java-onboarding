...
//7.포비가 이긴다면 1, 크롱이 이긴다면 2, 무승부는 0, 예외사항은 -1로 return 하도록 solution 메서드를 완성한다.
class Problem1 {
    public static int solution(List<Integer> pobi, List<Integer> crong) {
        int answer = Integer.MAX_VALUE;
        //1.책을 임의로 펼쳤을 때 옳은 값이 들어왔는지 확인한다.
        //왼쪽 페이지와 오른쪽 페이지의 숫자가 1차이 나는지 확인
        if(pobi.get(1) - pobi.get(0) != 1 || crong.get(1) - crong.get(0) != 1)
            return -1;
        //책의 처음이나 마지막 부분을 펼치지 않았는지 확인
        if(pobi.get(0) == 1 || pobi.get(1) == 400 || crong.get(0) == 1 || crong.get(1) == 400)
            return -1;
        //책을 펼쳤을 때 왼쪽은 홀수 오른쪽은 짝수 인지 확인
        for(int i = 0; i < 2; i++)
        {
            if(i == 0)
