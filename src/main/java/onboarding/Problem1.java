...
class Problem1 {
    public static int solution(List<Integer> pobi, List<Integer> crong) {
        ...
        for(Integer page:pobi) {
            pobiMax = Math.max(Math.max(getSumNumberOfDigits(page), getMultipleNumberOfDigits(page)),pobiMax);
        }

        for(Integer page:crong) {
            crongMax = Math.max(Math.max(getSumNumberOfDigits(page), getMultipleNumberOfDigits(page)),crongMax);
        }
