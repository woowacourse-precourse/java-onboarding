...
public class Problem2 {
    ...
    static String pro(String cryptogram) {
        boolean[] isDuplicated = new boolean[cryptogram.length()];  // 연속으로 중복되는 문자의 인덱스를 기록
        String s = "";
        ...
        for(int i = 1; i < cryptogram.length(); i++) {
            ...
            else isDuplicated[i] = true;    isDuplicated[i - 1] = true;
        }
        ...
        if(s.length() == cryptogram.length()) return s; // input 의 길이와 output 의 길이가 같으면 더 이상 해독팔 필요가 없다
        return pro(s);  // 재귀 호출
    }
}
