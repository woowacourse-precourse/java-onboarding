...
- **리팩터링_v2**
  - 함수형 프로그래밍 패러다임 적용해보기
    - 각각을 작은 단위의 함수로 쪼개기
    - 함수는 입력값에 대해 출력값을 돌려주는 구조로 제작(void 메소드 피하기)
  - solution의 매개변수 들을 Recommendation 클래스의 final 필드화해 활용
  - user_score는 변화하는 값이므로 필드에서 제외
    - 메소드들이 user_score를 매개변수로 받아 작업을 거쳐 return 값으로 이어지도록 설계
  - friends_map은 다른 메소드에서 쓰이고 변하지 않으므로 필드화 (리팩터링_v1 b 필드 삭제 revert)
