...
public class InputValueValidator {
    ...
    // 전체 검증
    public static void validateInputValue(String user, List<List<String>> friends, List<String> visitors) {
    ...
    // 문자열 길이 검증
    private static boolean isNotValidLength(String name) {
        int length = name.length();
        return MIN_VALUE > length || length > MAX_VALUE_FOR_STRING;
    }

    // 아이디는 소문자만!
    private static boolean notOnlyContainsLowerCase(String name) {
        return !name.matches("^[a-z]*$");  // 소문자만 있을 때 true를 반환하는 정규식이라고 한다.
    }

    // friends 리스트/배열 검증
    private static void validateFriendsList(List<List<String>> friendsList) {
    ...
    // visitor 리스트/배열 검증
    private static void validateVisitorList(List<String> visitorList) {
