...
```
1. 문자열을 입력받고 돈다. 
2. 문자가 대문자인지 소문자인지 확인하고 dto에 담는다. 
3. dto에서 끝 문자, 현재 문자, 첫 문자를 가져와서 끝 문자에서 현재 문자를 뺀 후, 첫 문자를 더한다. 
ex) 현재 문자 : C -> Z - C + A = X
...
### isUpperCase
- 문자가 대문자인지 확인하는 메서드
- [x] 대문자라면 Dto에 첫번째 대문자, 현재 문자, 마지막 대문자를 넣는다. 소문자도 똑같은 방식으로 넣는다. 

### CharDto
- 문자에 대한 정보를 가진 객체다. 
- [x] 첫 문자와 현재 문자, 끝 문자를 가진다.
