...
# Problem 2
### 문제 풀이 방법,
#### 스택 구조를 활용한다. 문자열을 순회하면서 현재 보고있는 문자를 스택에 넣을 때 스택 최상단에 있는 문자와 일치하는지의 여부를 판단한다. 만약 일치하는 문자가 스택의 최상단에 있다면 pop , 없다면 append(?)를 진행한다.  이후 스택에 남아있는 문자를 순서대로 출력하면된다.
- [ ] 스택의 최상단 문자를 확인하는 기능
- [ ] 스택의 최상단 문자와 현재 보고있는 문자를 비교하는 기능
- [ ] 비교한 문자가 같으면 스택의 최상단 문자를 삭제하는 기능
